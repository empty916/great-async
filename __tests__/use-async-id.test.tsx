...
test('id + swr: second mount hits cache and skips loading flash', async () => {
  ...
  // Second mount — same id, ttl not expired → no loading state, data from cache
  render(<App />);
  expect(screen.queryByRole('loading')).toBeNull();
  expect(screen.getByRole('app')).toHaveTextContent('value=1');

  // Background revalidation will eventually bump callCount
...
test('id without ttl/capacity does not survive unmount', async () => {
  ...
  const App = () => {
    const { loading, data } = useAsync(fetchData, {
      id, // no ttl, no cacheCapacity → caching off
      swr: true,
    });
    if (loading) return <span role="loading">loading</span>;
    return <div role="app">value={data?.value}</div>;
  ...
  // Second mount must show loading because nothing was cached
  render(<App />);
  expect(screen.getByRole('loading')).toBeInTheDocument();
  await waitFor(() => screen.getByRole('app'));
...
test('cacheManager + swr: pre-populated manager skips loading flash on first mount', async () => {
  // Custom cache manager pre-populated with data — useAsync should peek it
  // before flipping into the loading state, so the user never sees a flash.
...
test('IdCacheManager.clear(id) wipes cross-mount cache', async () => {
  ...
  // Cache cleared → second mount must show loading again
  render(<App />);
  expect(screen.getByRole('loading')).toBeInTheDocument();
  await waitFor(() => screen.getByRole('app'));
  expect(callCount).toBe(2);
